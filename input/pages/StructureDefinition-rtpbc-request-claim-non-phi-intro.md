...
In cases where the element to be masked is required by the base Claim resource, the Data Absent Reason extension is to be used with the code, "masked". §PHI-CLM-1:To conform with the Non-PHI Coverage profile, the Data Absent Reason extension **SHALL** be used in the Claim.patient and Claim.insurance.coverage elements, populated with the code, "masked".§
...
- Geographic area: Use the preferredPharmacyPostalCode extension in Claim.provider. §MS-CLM-1:The preferredPharmacyPostalCode extension **MAY** be repeated to specify multiple postal codes.§ §MS-CLM-2:An RTPBC Responder **SHALL** consider all preferredPharmacyPostalCode values submitted in a request.§

### Support for an anonymous patient identifier

§PHI-CLM-2:The patient.identifier element in this profile is available to be populated but **SHALL** contain only non-personally-identifiable codes, such as an account or user ID assigned during an anonymous interaction with the server.§
